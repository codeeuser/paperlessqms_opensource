...
class TokenCallPage extends StatefulWidget {
  ...
  const TokenCallPage({super.key, required this.prefs, required this.agent});
...
class _TokenCallPageState extends State<TokenCallPage> {
  ...
  Future<void> _initialize() async {
    ...
    int? terminalId = widget.agent.queueTerminal?.id;
    if (terminalId!=null) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          leading: IconButton(
            ...
            onPressed: () {
              Utils.pushAndRemoveUntilPage(
                  context,
                  WayScreen(prefs: widget.prefs, agent: widget.agent),
                  'WayScreen');
            },
          ),
          actions: [
        ...
        body: Consumer<AppAbsorb>(builder: (context, appAbsorb, child) {
              ...
              builder: (context, state) {
                ...
                return BlocBuilder<TokenBloc, TokenState>(
                  builder: (context, state) {
                    ...
                    } else if (state is TokenLoadedCallState){
                          ...
                          child: Column(
                            children: [
                              Padding(
                                ...
                                child: Text(
                                    S.of(context).selectOneOfTheItemsBelow),
                              ),
                              Row(
  ...
  Widget _buildItem(TokenIssuedModel issued, AppAbsorb appAbsorb) {
    bool isContain = statues4Call.contains(issued.statusCode);
    return ListTile(
      ...
      onTap: () async {
        if (isContain==false || issued.statusCode == StatusCode.cancel) {
          Utils.overlayInfoMessage(msg: S.of(context).noAction);
          return;
        }
        Utils.statusQueue(issued, _queueTerminal);
  ...
  Future<void> _findIssuedList() async {
    ...
    if (department!=null) {
      ...
      context.read<TokenBloc>().add(
        TokenByDepartmentWithStatusEvent(department: department, statues: statues4Call, reset: false)
      );
    }
  }
  ...
  Future<void> _showAction(TokenIssuedModel issued) async {
    ...
    String? result = await showModalActionSheet<String>(
      ...
      actions: [
        if (issued.statusCode != StatusCode.cancel) ...[
          SheetAction(
            label: S.of(context).callToken,
    ...
    if (result == userInfo) {
      Utils.pushPage(
          context,
          UserInfoScreen(prefs: widget.prefs, issued: issued),
          'UserInfoScreen');
      return;
    }

    if (result == Status.onqueue) {
      Utils.statusQueue(issued, _queueTerminal);
      await Utils.updateTokenIssued(
        context,
        appAbsorb,
