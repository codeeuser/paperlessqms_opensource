...
class DepartmentRepository {
  ...
  const DepartmentRepository({required this.context, required this.prefs});

  static RestConnectService restConnectService = locator<RestConnectService>();

  Future<dynamic> saveDepartment(QueueDepartmentModel? department) async{
    if (department==null) return null;
    final result = await restConnectService.postMethod(
      context, 
      Constants.apiDepartment,
    ...
    return QueueDepartmentModel.fromMap(result);
  }

  Future<dynamic> updateDepartment(QueueDepartmentModel? dep) async{
    if (dep==null || dep.id==null) return null;
    final result = await restConnectService.putMethod(
      context, 
      '${Constants.apiDepartment}/${dep.id}',
      headers: PrefsUtils.getHeader(prefs),
      body: jsonEncode(dep.toJson())
    );
    final error = ServerErrorModel.fromMap(result);
    if (error.data!=null) return error;
    return QueueDepartmentModel.fromMap(result);
  ...
  Future<dynamic> deleteDepartment(QueueDepartmentModel? dep) async{
    if (dep==null || dep.id==null) return null;
  ...
  Future<dynamic> getQueueDepartmentList(ProfileBizModel profileBiz, {bool? enable}) async{
    String filter = (enable!=null)?'&enable.equals=$enable':'';
    dynamic checkResult = await restConnectService.getMethod(
      context, '${Constants.apiDepartment}?profileBizId.equals=${profileBiz.id}$filter',
      headers: PrefsUtils.getHeader(prefs));
    final error = ServerErrorModel.fromMap(checkResult);
    if (error.data!=null) return error;
    if (checkResult.isNotEmpty){
      List<QueueDepartmentModel> list = [];
      for (Map e in checkResult) {
        QueueDepartmentModel queueDepartment = QueueDepartmentModel.fromMap(e);
        list.add(queueDepartment);
      }      
      return list;
    }
    return null;
  ...
  Future<dynamic> getQueueDepartment(int? id) async{
    if (id==null){
      return null;
    }    
    dynamic checkResult = await restConnectService.getMethod(
      context, '${Constants.apiDepartment}/$id',
      headers: PrefsUtils.getHeader(prefs));
    final error = ServerErrorModel.fromMap(checkResult);
    if (error.data!=null) return error;
    if (checkResult!=null){
      QueueDepartmentModel queueDepartment = QueueDepartmentModel.fromMap(checkResult);
