...
import 'package:admin/screens/way_screen.dart';
...
class _MorePageState extends State<MorePage> {
      ...
      child: SingleChildScrollView(
        child: SizedBox(
          ...
          child: Column(
            children: [
              ListTile(
                title: Text(S.of(context).setting.toUpperCase(),
                    style: styleHeader),
                tileColor: colorHeader,
              ),
              ListTile(
                leading: const Icon(CupertinoIcons.refresh_thin,
              ...
              ListTile(
                title: Text(S.of(context).appearance.toUpperCase(),
                    style: styleHeader),
                tileColor: colorHeader,
              ),
              Consumer<AppTheme>(builder: (context, appTheme, _) {
                IconData icon = (appTheme.mode == ThemeMode.dark)
                    ? CupertinoIcons.moon
              ...
              ListTile(
                title: Text(S.of(context).account.toUpperCase(),
                    style: styleHeader),
                tileColor: colorHeader,
              ),
              ListTile(
                ...
                subtitle: Text('${_account?.login} | ${_account?.email}',
                ...
                trailing: IconButton(
                  ...
                  onPressed: () {
                    Utils.pushPage(
                        context,
                        ProfileUserScreen(prefs: widget.prefs),
                        'ProfileUserScreen');
                  },
                ),
              ),
              ...
              ListTile(
                ...
                trailing: IconButton(
                  ...
                  onPressed: () {
                    Utils.pushPage(
                        context,
                        PasswordChangeScreen(prefs: widget.prefs),
                        'PasswordChangeScreen');
                  },
                ),
              ),
              ListTile(
                title: Text(S.of(context).moreInformation.toUpperCase(),
                    style: styleHeader),
                tileColor: colorHeader,
              ),
              BlocBuilder<BizBloc, BizState>(
              ...
              ListTile(
                leading:
                    const Icon(CupertinoIcons.info, semanticLabel: 'App Name'),
  ...
  Future<void> _resetAction() async {
    final result = await showOkCancelAlertDialog(
      context: context,
      title: S.of(context).resetTokenNumber,
      message: S.of(context).wantResetTokenNumber,
    );
    if (result == OkCancelResult.ok) {
      bool reset = true;
      context.read<TokenBloc>().add(TokenResetNumberEvent(reset: reset));
    } else {
