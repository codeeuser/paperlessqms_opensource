...
describe('Auth JWT', () => {
  ...
  describe('Get Token', () => {
    ...
    it('should return token from session storage if local storage is empty', () => {
      sessionStorage.setItem('qms-authenticationToken', JSON.stringify('sessionStorageToken'));
      const result = service.getToken();
      expect(result).toEqual('sessionStorageToken');
    });

    it('should return token from localstorage storage', () => {
      localStorage.setItem('qms-authenticationToken', JSON.stringify('localStorageToken'));
      const result = service.getToken();
